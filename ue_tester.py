...
def save_ping_results(ping_output, args):
    if ping_output:
        print(ping_output)
    time_values = re.findall(r'time=([0-9.]+)', ping_output)

    results_dir = find_results_dir(args.scheduler_file_name)
    cqi = CQI.get().as_string()
    with open(f'{results_dir}/{cqi}_ping_results.csv', 'w') as f:
        f.write("time(ms)\n")
        f.write('\n'.join(time_values))

def save_iperf_results(iperf_output, args):
    results_dir = find_results_dir(args.scheduler_file_name)
    cqi = CQI.get().as_string()
    with open(f'{results_dir}/{cqi}_iperf_results.csv', 'w') as f:
...
async def run_tests_for_urlcc(control, imei, args):
    print("\nURLCC: Running ping and iperf tests simultaneously")
    ...
    ping_ok, ping_output = await control.execute_command(
        imei,
        "cat /home/sierra/ping_results.txt",
        timeout=60.0
    )

    if not ping_ok:
        print("WARNING - Ping might have failed")
    
    save_ping_results(ping_output, args)


    iperf_ok, iperf_output = await control.execute_command(
        imei,
        "cat /home/sierra/iperf_results.txt",
        timeout=60.0
    )

    if not iperf_ok:
        print("WARNING - iperf might have failed")

    save_iperf_results(iperf_output, args)
