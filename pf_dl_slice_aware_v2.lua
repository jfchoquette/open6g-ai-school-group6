-- DL eMBB/URLLC Coexistence Scheduler (v2) with per-class OLLA MCS adaptation.
-- v2 additions over v1:
--   * Smallest-fit RB allocation for URLLC (preserves large blocks for eMBB)
--   * URLLC RB budget cap (max 50% of free RBs) to prevent eMBB starvation
--   * Deadline violation tracking in debug output
-- Every slot: retx → URLLC new data (smallest-fit) → eMBB new data (PF, largest block).
-- Traffic class determined by fiveQI (3GPP TS 23.501 Table 5.7.4-1).
-- Use: export LUA_SCHED=/path/to/pf_dl_slice_aware_v2.lua
...
function compute_dl_allocations(metrics_ptr, n_ues, total_rbs, min_rbs, rb_mask_string)
    ...
    for _, entry in ipairs(urllc_ues) do
        ...
        -- Smallest-fit: pick the tightest block that can hold `needed` RBs
        local s = find_smallest_free(mask, needed, m.bwp_start, m.bwp_size)
        if s >= 0 then
