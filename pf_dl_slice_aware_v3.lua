-- DL eMBB/URLLC Coexistence Scheduler (v3) with per-class OLLA MCS adaptation.
-- v3 additions over v2:
--   * Frequency-selective URLLC allocation using channel_mag_per_rb[]
--     Picks the contiguous block with highest average channel magnitude,
--     maximizing URLLC reliability on the best RBs.
-- Retained from v2: budget cap, deadline logging, per-class OLLA.
-- Every slot: retx → URLLC new data (best-channel fit) → eMBB new data (PF, largest block).
-- Traffic class determined by fiveQI (3GPP TS 23.501 Table 5.7.4-1).
-- Use: export LUA_SCHED=/path/to/pf_dl_slice_aware_v3.lua
