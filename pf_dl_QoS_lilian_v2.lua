...
function compute_dl_allocations(metrics_ptr, n_ues, total_rbs, min_rbs, rb_mask_string)
    ...
    -- QoS configuration
    -----------------------------------------------------------------------
    local URLLC_LATENCY_TARGET_US = 50000    -- 50 ms (ping)
    local EMBB_TARGET_THR_BPS   = 80e6     -- 80 Mbps, throughput is in bits/s (iperf)
    ...
    for _, entry in ipairs(urllc_ues) do
        ...
        local remain = free_now - used_urllc
        if remain < min_rbs then
            break
        end

        if entry.hol >= URLLC_LATENCY_TARGET_US then
    ...
    -----------------------------------------------------------------------
    -- Phase 4: give the rest to eMBB
    --
    -- If there is one eMBB UE, it can use the whole remaining budget.
    -- If there are multiple eMBB UEs, split the budget fairly according
    -- to throughput deficit.
    -----------------------------------------------------------------------
    local embb_budget = count_free_rbs(mask, 0, total_rbs)

    if embb_budget >= min_rbs and #embb_ues > 0 then
        ...
        for pos, entry in ipairs(embb_ues) do
            local m = metrics[entry.idx]
            ...
            if #embb_ues == 1 or deficit_sum <= 0.0 then
                grant_budget = remain
            else
                grant_budget = math.floor(embb_budget * (entry.deficit / deficit_sum))

                if pos == #embb_ues then
                    grant_budget = remain
                end
